...
{
	std::cout << "Host_System is starting simulation...." << std::endl;// Hybrid Change
	...
	if (preconditioning_required) {
		std::cout << "Preconditioning is required..." << std::endl;// Hybrid Change
	...
	std::cout << "Exiting Host_System::Start_simulation()" << std::endl;// Hybrid Change
}
...
{
	std::cout << "Host_System::Report_results_in_XML" << std::endl;// Hybrid Change
	std::string tmp;
	tmp = ID();
...
{
	std::cout << "Getting workload statistics...." << std::endl;// Hybrid Change
